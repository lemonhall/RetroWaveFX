...
RetroWaveFX.Audio = {
    ...
    _registerDefaultSounds() {
        ...
        // --- START OF NEW BATCH (30 SOUNDS) ---

        // Menu Navigate Up/Down (distinct sounds might be better, here's one)
        this.registerSound('menuNavigate', {
            description: 'Short blip for menu navigation.',
            emoji: '⬆️',
        ...
        this.registerSound('menuSelect', {
            description: 'Confirmation sound for menu selection.',
            emoji: '✅',
            category: 'UI/Interaction'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
        ...
        // Menu Back/Cancel
        this.registerSound('menuBack', {
            description: 'Sound for going back or canceling.',
            emoji: '↩️',
            category: 'UI/Interaction'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
            ...
            osc.frequency.linearRampToValueAtTime(220, now + duration * 0.8); // A3
            gainNode.gain.setValueAtTime(0.2, now);
            gainNode.gain.exponentialRampToValueAtTime(0.001, now + duration);
            osc.start(now);
            osc.stop(now + duration);
        ...
        // Typing Sound (Single Key)
        this.registerSound('typingKey', {
            description: 'Single typewriter key sound.',
            emoji: '⌨️',
            category: 'UI/Interaction'
        }, (audioCtx) => {
             const noiseBuffer = audioCtx.createBuffer(1, audioCtx.sampleRate * 0.05, audioCtx.sampleRate);
             const output = noiseBuffer.getChannelData(0);
             for(let i=0; i<50; i++) output[i] = Math.random() * 0.4 - 0.2; // Short noise burst
             const noiseSource = audioCtx.createBufferSource();
             noiseSource.buffer = noiseBuffer;
             const filter = audioCtx.createBiquadFilter();
             filter.type = 'highpass';
        ...
        // Space Ambience Loop (needs external stop control)
        this.registerSound('spaceAmbience', {
            description: 'Low, droney space ambience.',
            emoji: '🌌',
            ...
            osc2.type = 'sine';
            osc2.frequency.value = 57; // Slightly detuned
            filter.type = 'lowpass';
            filter.frequency.value = 120;
            lfo.type = 'sine';
            lfo.frequency.value = 0.1; // Very slow modulation
            lfoGain.gain.value = 20; // Modulate filter freq slightly
            lfo.connect(lfoGain).connect(filter.frequency);

            osc1.connect(filter);
            osc2.connect(filter);
            filter.connect(gainNode).connect(audioCtx.destination);
            gainNode.gain.value = 0.15; // Constant low volume

            osc1.start(audioCtx.currentTime);
            osc2.start(audioCtx.currentTime);
            lfo.start(audioCtx.currentTime);
            // Needs external stop for osc1, osc2, lfo
        });

        // Alien Chatter (short burst)
        this.registerSound('alienChatter', {
            description: 'Short burst of alien-like chatter.',
            emoji: '👽',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
            ...
            osc.type = 'sawtooth';
            lfo.type = 'sine';
            lfo.frequency.value = 25; // Fast modulation
            lfoGain.gain.value = 300; // Modulation depth for frequency
            lfo.connect(lfoGain).connect(osc.frequency);
            osc.connect(gainNode).connect(audioCtx.destination);
            const now = audioCtx.currentTime;
            const duration = 0.3;
            osc.frequency.setValueAtTime(500, now); // Base frequency
            gainNode.gain.setValueAtTime(0.2, now);
            gainNode.gain.exponentialRampToValueAtTime(0.01, now + duration);
            osc.start(now);
            osc.stop(now + duration);
        ...
        // Force Field Hum Loop (needs external stop)
        this.registerSound('forceFieldHum', {
            description: 'Steady energy field hum.',
            emoji: '✨',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const osc1 = audioCtx.createOscillator();
            ...
            osc1.frequency.value = 220; // A3
            osc2.type = 'sine';
            osc2.frequency.value = 221; // Slightly detuned for shimmer
            filter.type = 'bandpass';
            filter.frequency.value = 1000;
            filter.Q.value = 1;
            osc1.connect(gainNode);
            osc2.connect(gainNode);
            gainNode.connect(filter).connect(audioCtx.destination);
            gainNode.gain.value = 0.1; // Constant low volume

            osc1.start(audioCtx.currentTime);
            osc2.start(audioCtx.currentTime);
            // Needs external stop for osc1, osc2
        });

        // Force Field Impact
        this.registerSound('forceFieldImpact', {
            description: 'Sharp impact on an energy field.',
            emoji: '💥',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
        ...
        // Ricochet Sound
        this.registerSound('ricochet', {
            description: 'Bullet ricochet sound.',
            emoji: '💫',
        ...
        // Sword Swing/Swoosh
        this.registerSound('swordSwing', {
            description: 'A sharp whoosh of a sword swing.',
            emoji: '⚔️',
            category: 'Game/Action'
        }, (audioCtx) => {
        ...
        // Sword Hit Flesh
        this.registerSound('swordHitFlesh', {
            description: 'A wet thud sound of hitting flesh.',
            emoji: '🩸',
            category: 'Game/Action'
        }, (audioCtx) => {
            const noiseSource = audioCtx.createBufferSource();
            const noiseBuffer = audioCtx.createBuffer(1, audioCtx.sampleRate * 0.15, audioCtx.sampleRate);
            const output = noiseBuffer.getChannelData(0);
            for(let i=0; i<output.length; i++) output[i] = Math.random() * 0.8 - 0.4;
            noiseSource.buffer = noiseBuffer;
            const filter = audioCtx.createBiquadFilter();
            filter.type = 'lowpass';
        ...
        // Arrow Shot Sound
        this.registerSound('arrowShot', {
            description: 'The "thwip" of an arrow being fired.',
            emoji: '🏹',
            category: 'Game/Action'
        }, (audioCtx) => {
            const noiseSource = audioCtx.createBufferSource();
            const noiseBuffer = audioCtx.createBuffer(1, audioCtx.sampleRate * 0.1, audioCtx.sampleRate);
            const output = noiseBuffer.getChannelData(0);
            for (let i=0; i<output.length; i++) output[i] = Math.random() * 0.5 - 0.25;
            noiseSource.buffer = noiseBuffer;
            const filter = audioCtx.createBiquadFilter();
            filter.type = 'highpass';
        ...
        // Arrow Hit Target Sound
        this.registerSound('arrowHit', {
            description: 'The sound of an arrow hitting a target.',
            emoji: '🎯',
            category: 'Game/Action'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
        ...
        // Buff Activated Sound
        this.registerSound('buffActivate', {
            description: 'Sound for a positive status effect.',
            emoji: '🌟',
            category: 'Game/Action'
        }, (audioCtx) => {
            const gainNode = audioCtx.createGain();
            ...
            const freqs = [659.25, 880, 1046.50]; // E5, A5, C6
        ...
        // Debuff Applied Sound
        this.registerSound('debuffApplied', {
            description: 'Sound for a negative status effect.',
            emoji: '💀',
            category: 'Game/Action'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
            ...
            osc.frequency.linearRampToValueAtTime(207.65, now + duration * 0.8); // Ab3
            gainNode.gain.setValueAtTime(0.25, now);
            gainNode.gain.exponentialRampToValueAtTime(0.01, now + duration);
            osc.start(now);
            osc.stop(now + duration);
        ...
        // Potion Drink Sound
        this.registerSound('potionDrink', {
            description: 'Glugging sound of drinking a potion.',
            emoji: '🧪',
            category: 'Game/Action'
        }, (audioCtx) => {
            const noiseSource = audioCtx.createBufferSource();
            ...
            filter.Q.value = 8; // Resonant gurgle
            const gainNode = audioCtx.createGain();
            const now = audioCtx.currentTime;
            gainNode.gain.setValueAtTime(0, now);
            gainNode.gain.linearRampToValueAtTime(0.4, now + 0.1);
            gainNode.gain.linearRampToValueAtTime(0, now + 0.5);
            // Modulate filter slightly for bubbling effect
            const lfo = audioCtx.createOscillator();
            const lfoGain = audioCtx.createGain();
            lfo.type = 'square';
        ...
        // Heavy Footstep on Stone
        this.registerSound('stepStoneHeavy', {
            description: 'Heavy footstep on a stone surface.',
            emoji: '🪨',
            category: 'Footsteps'
        }, (audioCtx) => {
             const osc = audioCtx.createOscillator();
             ...
             const filter = audioCtx.createBiquadFilter();
             osc.type = 'sine';
             osc.frequency.setValueAtTime(100, audioCtx.currentTime); // Lower pitch
             filter.type = 'lowpass';
        ...
        // Footstep on Grass/Dirt
        this.registerSound('stepGrass', {
            description: 'Soft rustle of walking on grass.',
            emoji: '🌿',
            category: 'Footsteps'
        }, (audioCtx) => {
             const noiseSource = audioCtx.createBufferSource();
             ...
             const filter = audioCtx.createBiquadFilter();
             filter.type = 'bandpass';
             filter.frequency.value = 2000; // Higher freq focus
             filter.Q.value = 0.8;
             const gainNode = audioCtx.createGain();
             gainNode.gain.setValueAtTime(0.25, audioCtx.currentTime); // Quieter
        ...
        // Rain Loop (needs external stop)
        this.registerSound('rainLoop', {
            description: 'Continuous rain sound.',
            emoji: '🌧️',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const noiseSource = audioCtx.createBufferSource();
            ...
            const filter = audioCtx.createBiquadFilter();
            filter.type = 'highpass';
            filter.frequency.value = 1000; // Filter out lows for rain sound
            const gainNode = audioCtx.createGain();
            gainNode.gain.value = 0.15; // Adjust volume as needed
            noiseSource.connect(filter).connect(gainNode).connect(audioCtx.destination);
            noiseSource.start(audioCtx.currentTime);
            // Needs external stop for noiseSource
        });

        // Thunder Clap Sound
        this.registerSound('thunderClap', {
            ...
            const filter = audioCtx.createBiquadFilter();
            filter.type = 'lowpass';
            filter.frequency.setValueAtTime(1000, audioCtx.currentTime); // Start brighter
            filter.frequency.exponentialRampToValueAtTime(50, audioCtx.currentTime + 1.2); // Rumble down
            filter.Q.value = 5;
            const gainNode = audioCtx.createGain();
            const now = audioCtx.currentTime;
            gainNode.gain.setValueAtTime(0, now);
        ...
        // Robot Boot Up Sound
        this.registerSound('robotBootUp', {
            description: 'Sound of a robot starting up.',
            emoji: '🤖',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const gainNode = audioCtx.createGain();
            ...
            gainNode.gain.setValueAtTime(0, now);
            gainNode.gain.linearRampToValueAtTime(0.25, now + 0.1);
            gainNode.gain.setValueAtTime(0.25, now + totalDuration - 0.1);
            gainNode.gain.linearRampToValueAtTime(0, now + totalDuration);

            // Rising sweep
            const oscSweep = audioCtx.createOscillator();
            oscSweep.type = 'sawtooth';
            oscSweep.frequency.setValueAtTime(100, now);
            oscSweep.frequency.linearRampToValueAtTime(1000, now + totalDuration * 0.7);
            oscSweep.connect(gainNode);
            oscSweep.start(now);
            oscSweep.stop(now + totalDuration);

            // Clicks
            const freqs = [1500, 1800, 2200];
            const times = [0.2, 0.4, 0.6];
            for(let i=0; i<freqs.length; i++){
                 const oscClick = audioCtx.createOscillator();
                 oscClick.type = 'square';
                 oscClick.frequency.value = freqs[i];
                 oscClick.connect(gainNode);
                 oscClick.start(now + times[i]);
                 oscClick.stop(now + times[i] + 0.03);
            }
        });

        // Robot Shutdown Sound
        this.registerSound('robotShutdown', {
            description: 'Sound of a robot shutting down.',
            emoji: '🔌',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const gainNode = audioCtx.createGain();
            ...
            oscSweep.frequency.exponentialRampToValueAtTime(50, now + totalDuration * 0.9);
            oscSweep.connect(gainNode);
            oscSweep.start(now);
            oscSweep.stop(now + totalDuration);
            ...
            noiseSource.buffer = noiseBuffer;
            const noiseGain = audioCtx.createGain();
            noiseGain.gain.setValueAtTime(0.2, now + totalDuration * 0.8);
            noiseGain.gain.exponentialRampToValueAtTime(0.01, now + totalDuration);
            noiseSource.connect(noiseGain).connect(audioCtx.destination);
            noiseSource.start(now + totalDuration * 0.8);
            noiseSource.stop(now + totalDuration);
        });

        // Small Creature Scurry
        this.registerSound('creatureScurry', {
            description: 'High-pitched scuttling sound.',
            emoji: '🐁',
            category: 'Game/Action'
        }, (audioCtx) => {
             const now = audioCtx.currentTime;
             ...
             for (let t = 0; t < duration; t += interval) {
                 ...
                 noiseSource.stop(now + t + 0.02);
             }
        });

        // Large Creature Roar
        this.registerSound('creatureRoar', {
            description: 'Low-pitched roar of a large creature.',
            emoji: '🦖',
            category: 'Game/Action'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
            ...
            const k = 50, n_samples = 44100, curve = new Float32Array(n_samples);
            ...
            osc.type = 'sawtooth';
            osc.frequency.setValueAtTime(100, audioCtx.currentTime); // Low base
            osc.frequency.linearRampToValueAtTime(70, audioCtx.currentTime + 0.8); // Pitch drop
            osc.connect(distortion).connect(gainNode).connect(audioCtx.destination);

            const now = audioCtx.currentTime;
            const duration = 1.2;
            gainNode.gain.setValueAtTime(0, now);
            gainNode.gain.linearRampToValueAtTime(0.5, now + 0.2); // Swell in
            gainNode.gain.exponentialRampToValueAtTime(0.01, now + duration);
        ...
        // Countdown Beep
        this.registerSound('countdownBeep', {
            description: 'Regular interval beep for countdowns.',
            emoji: '⏱️',
        ...
        // Time Up/Fail Buzzer
        this.registerSound('failBuzzer', {
            description: 'Harsh buzzer sound for failure.',
            emoji: '🚫',
            category: 'Alert/Status'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
            ...
            const duration = 0.5;
            gainNode.gain.setValueAtTime(0.4, now);
            gainNode.gain.setValueAtTime(0.4, now + duration - 0.05); // Hold
            gainNode.gain.linearRampToValueAtTime(0, now + duration); // Abrupt stop
            osc.start(now);
            osc.stop(now + duration);
        });

        // Objective Complete Chime
        this.registerSound('objectiveComplete', {
            description: 'Pleasant chime for completing an objective.',
            emoji: '🏆',
            category: 'UI/Interaction'
        }, (audioCtx) => {
            const gainNode = audioCtx.createGain();
        ...
        // Secret Found Jingle
        this.registerSound('secretFound', {
            description: 'Short, mysterious jingle for finding secrets.',
            emoji: '❓',
            category: 'UI/Interaction'
        }, (audioCtx) => {
            const gainNode = audioCtx.createGain();
            ...
            gainNode.gain.setValueAtTime(0.2, audioCtx.currentTime);
            gainNode.gain.exponentialRampToValueAtTime(0.0001, audioCtx.currentTime + 0.6);
            // Use a slightly unusual interval, e.g., Augmented chord C-E-G#
            const freqs = [523.25, 659.25, 830.61]; // C5, E5, G#5
        ...
        // Engine Start Sound
        this.registerSound('engineStart', {
            description: 'Sound of an engine starting up.',
            emoji: '🚗',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
            ...
            const now = audioCtx.currentTime;
            const duration = 1.0;
            // Pitch rises from low to idle
            osc.frequency.setValueAtTime(40, now);
            osc.frequency.linearRampToValueAtTime(80, now + duration * 0.8);
            // Filter opens up
            filter.frequency.setValueAtTime(100, now);
            filter.frequency.linearRampToValueAtTime(300, now + duration * 0.7);
            // Gain increases
            gainNode.gain.setValueAtTime(0.1, now);
            gainNode.gain.linearRampToValueAtTime(0.35, now + duration);
            osc.start(now);
            osc.stop(now + duration); // Stop after start-up sound
        });

        // Engine Stop Sound
        this.registerSound('engineStop', {
            description: 'Sound of an engine sputtering and stopping.',
            emoji: '🛑',
            category: 'Sci-Fi/Environment'
        }, (audioCtx) => {
            const osc = audioCtx.createOscillator();
            ...
            const now = audioCtx.currentTime;
            const duration = 0.8;
            // Pitch drops and maybe flutters
            osc.frequency.setValueAtTime(80, now);
            osc.frequency.exponentialRampToValueAtTime(30, now + duration * 0.9);
            // Gain fades out
            gainNode.gain.setValueAtTime(0.35, now);
            gainNode.gain.exponentialRampToValueAtTime(0.001, now + duration);

            // Add sputter with LFO on frequency briefly
            const lfo = audioCtx.createOscillator();
            const lfoGain = audioCtx.createGain();
            lfo.type = 'square';
            lfo.frequency.value = 15;
            lfoGain.gain.value = 10;
            lfo.connect(lfoGain).connect(osc.frequency); // Modulate briefly
            lfo.start(now + duration * 0.6);
            lfo.stop(now + duration * 0.8);
